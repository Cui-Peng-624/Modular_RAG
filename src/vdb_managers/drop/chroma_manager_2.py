...
class ChromaManager:
    ...
    def _get_vector_store(self, collection_name: str, persist_directory: str = None) -> Chroma:
        """根据 collection_name 初始化或加载 Chroma 向量存储"""
        if persist_directory is None:
            current_directory = os.getcwd()
            persist_directory = os.path.join(current_directory, f'ChromaVDB')
    ...
    def upload_pdf_file(self, file_path: str, chunk_size: int = 1000, chunk_overlap: int = 100, auto_extract_metadata: bool = True) -> None:
        ...
        documents_chunks = splitter.split_documents(documents) # list: [Document(metadata={'source': 'files/论文 - GraphRAG.pdf', 'page': 0}, page_content=), ...]

        # 提取元数据
        if auto_extract_metadata:
            metadata = extract_metadata_sync(file_path, documents_chunks)  # 调用同步版本的 extract_metadata # list: [{'category': '', 'keywords': ['', '', '', '', '']}, ...]
            # print("\n\n########################\n", metadata, "\n########################\n\n")
            # 将每个 chunk 的 category 和 keywords 进行分组
            categorized_chunks = {} # {'计算机': {'chunks': [Document(metadata={'source': 'files/论文 - GraphRAG.pdf', 'page': 0}, page_content=), [], ...], 'keywords': {'', '', '', '', ''}}}
            for chunk, meta in zip(documents_chunks, metadata):
        ...
        for category, data in categorized_chunks.items():
            # print("\n\n================\n", category, "\n================\n\n")
            # 更新元数据注册表
            self._update_metadata_registry([category], list(data["keywords"]))

            # 初始化对应的 vector_store
            vector_store = self._get_vector_store(collection_name=category)
    ...
    def upload_txt_file(self, file_path: str, chunk_size: int = 1000, chunk_overlap: int = 100) -> None:
        ...
        # 调用大模型提取元数据
        model = ModelManage()
        metadata = model.extract_metadata(file_path, documents_chunks)
        category = metadata.get("category", "未分类")
        keywords = metadata.get("keywords", [])

        # 更新元数据注册表
        self._update_metadata_registry(category, keywords)

        # 初始化对应的 vector_store
        vector_store = self._get_vector_store(collection_name=category)

        uuids = [str(uuid4()) for _ in range(len(documents_chunks))]
        vector_store.add_documents(documents=documents_chunks, ids=uuids, metadatas=[{"keywords": kw} for kw in keywords])
    ...
    def upload_directory(self, directory_path: str, chunk_size: int = 1000, chunk_overlap: int = 100) -> None:
        ...
        # 调用大模型提取元数据
        model = ModelManage()
        metadata = model.extract_metadata(directory_path, documents_chunks)
        category = metadata.get("category", "未分类")
        keywords = metadata.get("keywords", [])
